...
```bash
npx -y @sneeai/sneeai-agent@0.3.1 open "<Vite Local 地址>/canvas?mode=new"
```
...
插件在新的 Codex 任务中加载时会自动启动 `npx -y @sneeai/sneeai-agent@0.3.1 mcp`。MCP 启动前会自动拉起或复用同一个普通本地 Sneeai Agent，再通过本地配置把画布工具请求转发给它；多个 MCP 进程不会重复占用 Agent 端口。
