...
const sitePathSchema = z.string().regex(/^\/(?:canvas(?:\/[^/?#]+)?|image|video|prompts|assets)?(?:[?#].*)?$/);

/** Sneeai Agent 对外提供的工具名称。 */
...
const videoReferenceInputsSchema = z
    .object({
        images: z.array(generationInputObjectSchema).optional(),
        videos: z.array(generationInputObjectSchema).optional(),
        audios: z.array(generationInputObjectSchema).optional(),
    })
    .strict();

const workbenchVideoGenerateSchema = z
    .object({
        prompt: z.string(),
        model: z.string().optional(),
...
export const toolInputSchemas = {
    site_navigate: z.object({ path: sitePathSchema }),
